...
class qcs:
    ...
    def __init__(self, Heff: list, Input: list, Output: list, ratio=None):
        ...
        """
        self.__Heff_c = [Hi[0] for Hi in Heff]
        self.__Heff_v = [Hi[1:] for Hi in Heff]
        self.__Input = {}
        self.__ratio = {}
        self.__Frequency = {}
    ...
    def __Heff_Matrix(self, n_exc: int):
        ...
        """
        Heff_List = np.array(list(qcs.__HeffList[n_exc].values())) * self.__Heff_c
    ...
    def calculate_quantity(self, Quantity: str, tlist=0, zp=0):
        ...
        if self.__Heff_v != qcs.__Judge_Heff:
            ...
            qcs.__Judge_InOut = set(self.__Input.keys()).union(set(self.__Output.keys()))
            for n in range(0, n_exc + 1):
                self.__basis(n)
                if n != 0:
                    self.__prestore_HeffList(n)
                    self.__prestore_InOutList(n)
        elif set(self.__Input.keys()) <= qcs.__Judge_InOut and set(self.__Output.keys()) <= qcs.__Judge_InOut:
            pass
        else:
            qcs.__Judge_InOut = set(self.__Input.keys()).union(set(self.__Output.keys()))
        ...
        elif label == 3:  # one-to-multiple-same
            key_in = list(self.__Input.keys())[0]
        ...
        elif label == -4:  # multiple-to-multiple-different
            print("This function is not yet available")
        else:  # multiple-to-multiple-same
            print("This function is not yet available")
